...
foldername = '9mer_memb_simu_mu={0:}_Jb={1:}_Jm={2:}_Jteth={3:}_lipAfrac={4:}_L={5:}_D={6:}'.format(mu,Jb,Jmemb,Jteth,lipAfrac,L,D)
import os
                ...
                if rand()<0.5:     # do addition
                    ...
                    if Add:
                        for each in [tail1, tail2, tail3, tail4, center, tail5, tail6, tail7, tail8]:
                            'tether length relevant'
                            if each[0] in [0,1,2,3,4] and memb[each[1],each[2]]==1:
                                Add=False
                                break
                            else:
                                for dr in configuration_pool:
