...
foldername = '9mer_memb_simu_mu={0:}_Jb={1:}_Jmem={2:}_Jteth={3:}_obst_frac={4:}_tethfrac={5:}_L={6:}_D={7:}'.format(mu,Jb,Jmemb,Jteth,obstacle_frac,tether_frac,L,D)
import os
...
class GCMC_9mer_memb():
    ...
    def mc_sweep_9mer_memb(self, mu, Jb, Jmemb, Jteth, L, D):
        ...
        for sitee in random_memb_sites:
            ...
            j= sitee%L
            s1 = memb[i,j]
            if s1==0 or tethers[i,j]==1: pass
            else:
                s2, a, b = 0,0,0
                while s2==0 or s2==s1 or tethers[a,b]==1:
...
# THE following 14 lines are temporarily commented out    
#import sys
#mu = float(sys.argv[1])
#Jb = float(sys.argv[2])
#L = int(sys.argv[3])
...
#with open(foldername+'/9mer_memb_polymerpool_history','w') as f:
#    f.writelines('$')
#    f.writelines(str(memb_9mer.polymer_pool))
#    f.writelines('$')
    
#with open(foldername+'/9mer_memb_state_history','w') as k:
#    k.writelines('$')
#    listit=[]
...
#with open(foldername+'/9mer_memb_board_history','w') as f:
#    f.writelines('$')
#    Nums=[]
...
#with open(foldername+'/9mer_memb_tether_history','w') as f:
#    f.writelines('$')
#    Nums=[]
...
#with open(foldername+'/9mer_memb_polypool_length','w') as p:
...
#    memb_1mer.state_history.append(memb_1mer.state)
#    memb_1mer.polymer_pool_history.append(memb_1mer.polymer_pool)
#    memb_1mer.board_history.append(memb_1mer.board)
#    memb_1mer.tether_history.append(memb_1mer.tether)
    
    # writeout polymer pool after each sweep
    #with open(foldername+'/9mer_memb_polymerpool_history','a') as f:
    #    f.writelines(str(memb_9mer.polymer_pool))
    #    f.writelines('$')
        
    #with open(foldername+'/9mer_memb_state_history','a') as k:
    #    listit=[]
    #    for layer in memb_9mer.state:
    #        ceng = []
    #        for row in layer:
    ...
    #with open(foldername+'/9mer_memb_board_history','a') as f:
    #    Nums=[]
    #    for lin in memb_9mer.board:
    #        for num in lin:
    #            Nums.append(num)
    ...
    #with open(foldername+'/9mer_memb_tether_history','a') as f:
    #    Nums=[]
    #    for lin in memb_9mer.tether:
    #        for num in lin:
    #            Nums.append(num)
    ...
    #with open(foldername+'/9mer_memb_polypool_length','a') as p:
    #    p.writelines(str(len(memb_9mer.polymer_pool))+'\n')
    ...
    if itr%10000==0:
        figtitle = '9mer_memb_mu={0:}_Jb={1:}_Jmem={2:}_Jteth={3:}\nobstacle={4:}_teth={5:}'.format(mu,Jb,Jmemb,Jteth,obstacle_frac,tether_frac)
        figname = foldername+'/9mer_memb_mu=-{0:}_Jb={1:}_Jmem={2:}_Jteth={3:}_obstacle={4:}_teth={5:}_L={6:}_D={7:}_swp={8:}.png'.format(mu,Jb,Jmemb,Jteth,obstacle_frac,tether_frac,L,D,itr)
